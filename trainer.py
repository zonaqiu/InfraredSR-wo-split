...
class Trainer():
    ...
    def train(self, current_epoch=0, is_init=False):
        ...
        for i_batch, sample_batched in enumerate(self.dataloader['train']):
            ...
            sr, S, T_lv3 = self.model(lr=lr, lrsr=lr_sr, ref=ref, refsr=ref_sr)
            # sr = self.model(lr=lr, lrsr=lr_sr, ref=ref, refsr=ref_sr)
            ...
            if (not is_init):
                ...
                # transferal perceptual loss 暂时不使用
        ...
        if (self.args.dataset == 'CUFED'):
            ...
            with torch.no_grad():
                ...
                for i_batch, sample_batched in enumerate(self.dataloader['test']['1']):
                    ...
                    sr, _, _ = self.model(lr=lr, lrsr=lr_sr, ref=ref, refsr=ref_sr)
    ...
    def test(self):
        ...
        for LR_filename in os.listdir(self.args.lr_path):
            ...
            self.model.eval()
            with torch.no_grad():
                sr, _, _ = self.model(lr=LR_t, lrsr=LR_sr_t, ref=Ref_t, refsr=Ref_sr_t)
                sr_save = (sr + 1.) * 127.5
                sr_save = np.transpose(sr_save.squeeze().round().cpu().numpy(), (1, 2, 0)).astype(np.uint8)
